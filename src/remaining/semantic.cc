...
{
    /* Your code here */
	//2 Main cases: check of type and of numbers (through the recursivity)	
	//#1 Type: Need to update the type if single node
	//	Send error and return false when different type
	//#2 Numbers: 3 cases to manage for formals and actuals (check type of last_expr)
	//	1- two same types except NULL (inspect the precedings, wait for recursivity 'call back')
	//	2- only one NULL (return false, different number between formals and actuals: 2 checks)
	//	3- both NULL (return true, end of recursivity)

	//#1 Update type on the actual and do the test
	actuals->last_expr->type = actuals->last_expr->type_check();
	if (formals->type != actuals->last_expr->type)
	{
		type_error(env->pos) << "Type discrepancy between formal and actual parameters. (chk_param)" << endl;
		return false;
	}

	//#2 Check number of params (actual vs formal) recursively and do recursively call of chk_param if needed
	if (actuals->preceding != NULL || formals->preceding != NULL)
	{
		if (actuals->preceding == NULL && formals->preceding != NULL)
		{
			//More formal than actual
			type_error(env->pos) << "More formal than actual parameters. (chk_param)" << endl;	
			return 0;
		}
		if (actuals->preceding != NULL && formals->preceding == NULL)
		{
			//More actual than formal
			type_error(env->pos) << "More actual than formal parameters. (chk_param)" << endl;	
			return 0;
		}
		//Recursive call
		chk_param(env,formals->preceding, actuals->preceding);
	}
	//End of the recursion when no if is triggered and return done
    return true;
}
                                ...
                                ast_expr_list *param_list)
{
	...
	if (s->tag == SYM_FUNC) {
		...
		formals = proc_s->last_parameter;
    }
	else {
		type_error(call_id->pos) << "Neither a function or a procedure (check_parameters)" << endl;
	}
	
	//Check if it is null (no parameters)
	if (formals == NULL)
	{
		if(param_list==NULL){
			//No problem (nb actual = nb formal = 0)
		}
		else {
			//Problem (0 != nb actual > nb formal = 0, should be equal)
			type_error(call_id->pos) << "More actual than formal parameters. (check_parameters)" << endl;
    	}
	}
	else
	{
		if(param_list==NULL){
			//Problem (0 = nb actual > nb formal != 0, should be equal)
			type_error(call_id->pos) << "More formal than actual parameters. (check_parameters)" << endl;
		}
		else{
			//No problem (formals & param_list != NULL, but can be not equal there)
			//Can check the parameter type disparency now and numbers recursively

			//Check if it's formals is a parameter list
			if(formals->tag != SYM_PARAM){
				// Problem: not a parameter list
				type_error(call_id->pos) << "Must be a parameter list\n";
    		}
   	
    		//Safe downcast to parameter symbol since it is a parameter list
    		parameter_symbol *param = formals->get_parameter_symbol();

			//Check type of formals and actuals
			if(!chk_param(call_id, param, param_list)){
				//Problem found in the recursivity
				//Errors message are also generated by chk_param.
   	 		}
		}
    }
}
